...
import {
  HelpCircle, Users, PlayCircle, FileText, Plus, QrCode, Sparkles, ArrowRight,
} from "lucide-react";
...
function DashboardPage() {
  const { user } = useAuth();
  const { t } = useI18n();
  const [stats, setStats] = useState<Stats>({ quizzes: 0, sessions: 0, participants: 0, questions: 0 });
  ...
  const cards = [
    ...
    { label: t("dash.participants"), value: stats.participants, icon: Users, color: "text-warning" },
    { label: t("dash.questions"), value: stats.questions, icon: HelpCircle, color: "text-primary-glow" },
  ];
  ...
  const actions = [
    ...
    { label: t("dash.startSession"), to: "/sessions", icon: PlayCircle },
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-muted-foreground mt-1">A clear view of your quizzes, sessions, and participants.</p>
      </div>

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {cards.map((c) => (
          <div key={c.label} className="rounded-2xl border border-border bg-card/60 backdrop-blur p-5 shadow-card hover:border-primary/30 transition-colors">
        ...
        <div className="rounded-2xl border border-border bg-card/60 p-5">
          ...
          {recent.length === 0 ? (
            ...
            <ul className="space-y-2">
              {recent.map((r) => (
                <li key={r.id} className="flex items-center justify-between rounded-xl bg-secondary/40 px-3 py-2.5">
                  <div>
                    <div className="text-sm font-medium">{r.title}</div>
                    <div className="text-xs text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</div>
                  </div>
                  <span className="text-xs px-2 py-1 rounded-full bg-primary/15 text-primary capitalize">{r.status}</span>
                </li>
              ))}
            </ul>
          )}
