...
const AI_STEPS: Step[] = [
  { icon: <Brain className="h-5 w-5" />,        label: "Understanding your topic",   detail: "Analysing subject matter and context…" },
  { icon: <Sparkles className="h-5 w-5" />,     label: "Generating questions",       detail: "Claude AI is crafting unique questions…" },
  { icon: <FileText className="h-5 w-5" />,     label: "Structuring answers",        detail: "Adding correct answers and distractors…" },
  { icon: <CheckCircle2 className="h-5 w-5" />, label: "Reviewing quality",          detail: "Checking accuracy and formatting…" },
  { icon: <Wand2 className="h-5 w-5" />,        label: "Almost ready",               detail: "Wrapping up your question set…" },
];
...
export function LoadingOverlay({
  ...
  useEffect(() => {
    if (!visible) { setAiStepIdx(0); return; }
  ...
  const subhint = isDriven ? (hint ?? `Step ${Math.min(activeIdx + 1, renderSteps.length)} of ${renderSteps.length}`) : "This usually takes 10–30 seconds";
      ...
      <div className="w-full max-w-sm mx-4 rounded-2xl border border-primary/30 bg-card shadow-2xl overflow-hidden">
        ...
        <div className="px-7 py-8 space-y-6">
          ...
          <div className="flex flex-col items-center gap-3 text-center">
            <div className="relative flex items-center justify-center h-16 w-16 rounded-2xl bg-primary/10 border border-primary/25">
              {allDone
                ? <CheckCircle2 className="h-8 w-8 text-success" />
                : <Sparkles className="h-8 w-8 text-primary animate-pulse" />}
              {!allDone && <span className="absolute inset-0 rounded-2xl border-2 border-primary/20 animate-ping" />}
            </div>
            <div>
              <p className="font-display font-bold text-lg text-foreground">
                {headline}{allDone ? "" : dots}
              </p>
          ...
          <div className="space-y-2.5">
            {renderSteps.map((step, i) => {
              ...
              return (
                ...
                >
                  <span className={`shrink-0 ${active ? "text-primary" : done ? "text-success" : "text-muted-foreground"}`}>
                    {done ? <CheckCircle2 className="h-5 w-5" /> : (step.icon ?? <Loader2 className={`h-5 w-5 ${active ? "animate-spin" : ""}`} />)}
                  </span>
                  ...
                  {active && (
                    <div className="ml-auto shrink-0 h-4 w-4 rounded-full border-2 border-primary/30 border-t-primary animate-spin" />
                  )}
                </div>
              );
            })}
