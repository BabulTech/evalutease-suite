    ...
    <div className="relative">
      ...
      {showBadge && (
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            <Lock className="size-3.5" /> {LIMIT_LABELS[feature]} reached, Upgrade
          </Link>
        </div>
      )}
    </div>
...
export function AiGate({ children, fallback }: AiGateProps) {
  ...
  return (
    <div className="rounded-2xl border border-dashed border-primary/30 bg-primary/5 p-6 text-center">
      <Zap className="mx-auto size-8 text-primary/50 mb-2" />
      ...
      >
        <Zap className="size-4" /> View Plans
      </Link>
    </div>
  );
}
...
// ─── Imperative hook ─────────────────────────────────────────────
// eslint-disable-next-line react-refresh/only-export-components -- hook co-located with the component it guards
