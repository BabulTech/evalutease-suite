-- Fix get_session_for_play to infer question type from data when the type column is NULL.
-- This handles questions created before the type column was populated.
-- Inference rules (matches client-side logic):
--   model_answer IS NOT NULL OR rubric IS NOT NULL          → long_answer
--   acceptable_answers IS NOT NULL AND array_length > 0     → short_answer
--   array_length(options, 1) = 2 AND correct_answer IN ('true','false') → true_false
--   otherwise                                               → mcq
